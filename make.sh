...
{
	multini --set "$KFEditorConf" 'ModPackages' 'ModPackages' 'ZedTernalReborn'
	multini --add "$KFEditorConf" 'ModPackages' 'ModPackages' 'UnofficialMod'
	multini --add "$KFEditorConf" 'ModPackages' 'ModPackages' 'UnofficialModZT'
	multini --set "$KFEditorConf" 'ModPackages' 'ModPackagesInPath' "$(cygpath -w "$MutSource")"
}

function get_zedternal ()
{
	local ZedternalGit='https://github.com/Erxandros/Zedternal-Reborn.git'
	local ZedternalDir="$MutSource/ZedTernalReborn"
...
function compiled ()
{
	test -f "$MutStructScript/UnofficialMod.u" && test -f "$MutStructScript/UnofficialModZT.u"
}
...
function brew ()
{
	...
	# CMD //C "$(cygpath -w "$KFEditor")" brewcontent -platform=PC ServerExt ServerExtMut -useunpublished
}
...
function generate_wsinfo () # $1: package dir
{
	...
	local Preview=$(cygpath -w "$MutPubContent/preview.png")
	local Tags=$(cat "$MutPubContent/tags.txt")
	local PackageDir=$(cygpath -w "$1")
...
Difficulty=\"0\"
GameLength=\"0\"
Mutators=\"UnofficialMod.UnofficialModMut\"
Args=\"\"" > "$MutTestingIni"
}
